...
void texture::render(const rs2::frameset &frames, int window_width, int window_height) {
...
void texture::render(const rs2::video_frame &frame, const rect &r)
{
    upload(frame);
...
void texture::show(const rect &r) const
{
    if (!gl_handle)
        return;
...
bool texture::can_render(const rs2::frame &f) const
{
...
rect texture::calc_grid(float2 window, int streams)
{
...
std::vector<rect> texture::calc_grid(float2 window, std::vector<rs2::video_frame> &frames)
{
...
void glgl::draw_pointcloud(float width, float height, glfw_state &app_state, rs2::points &points)
{
    if (!points)
        return;
...
void glgl::register_glfw_callbacks(window &app, glfw_state &app_state)
{
